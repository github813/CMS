...
export class DriverFormComponent implements OnInit {
  ...
  constructor(private router:Router,private formBuilder:FormBuilder,private rateDirverService:RateDirverService,private routerInfo: ActivatedRoute,public appComponent:AppComponent ) {
    //初始化表单
  ...
  findById(){
    this.rateDirverService.getRateDirverById(this.id).subscribe(ret =>{
      this.loading = false;
      if(ret.code == 0){
        let value ={
  ...
  //取消按钮
  cancel(){
    this.router.navigateByUrl('/user/rate/rate-driver');
  }
  ...
  saveNew(){
    ...
    let response:Observable<any> = this.rateDirverService.saveRateDirver(this.formModel.value);
  ...
  saveUpdate(){
    let response:Observable<Result> = this.rateDirverService.updateRateDirver(this.id,this.formModel.get("rate").value);
    response.subscribe(result =>{
      if(result.code == 0){
        this.showMsg("操作成功","success");
        //this.router.navigateByUrl("/system/staff");
